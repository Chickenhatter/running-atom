...
var zim = false
func _ready() -> void:
	velocity = Vector2(0, 10).normalized() * 200
...
func _physics_process(delta: float) -> void:
	...
	if zim == true:
		...
		$"../../../Character/Node2D/main_hydrogen".look_at($connect_o_2.global_position)
	$CollisionPolygon2D.rotation_degrees += 5


func _on_connect_o_2_area_entered(area: Area2D) -> void:
